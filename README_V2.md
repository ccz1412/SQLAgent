...
- ✅ **双模型纠错架构**：API 生成 SQL + 本地 Llama 审查/纠错
...
│   ├── agent\              # Agent 核心
│   │   ├── sql_generator.py       # SQL 生成（支持本地/API）
│   │   ├── react_agent.py            # ReAct 推理（双模型纠错架构）
│   │   └── intent_detector.py      # 意图检测（SQL_QUERY/CHAT/REJECT）
│   ├── dialogue\            # 多轮对话
...
└─────────┼──────────────────┼──────────────────┼───────────────┘
          ...
          ▼                  ▼                  ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                      对话管理层（Dialogue Manager）                      │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  意图检测（Intent Detector）                               │    │
│  │  • SQL_QUERY → 进入 ReAct Agent                         │    │
│  │  • CHAT → 知识库回答（介绍自己、数据库、SQL 知识）    │    │
│  │  • REJECT → 礼貌拒绝无关问题                           │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                              │                                       │
│                              ▼                                       │
...
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │  生成阶段（Generation LLM）                               │    │
│  │  • 模型：API（智谱 AI glm-4）或本地 Llama-3.1-8B     │    │
│  │  • 输入：用户问题 + Schema + 对话历史                   │    │
│  │  • 输出：SQL 语句                                      │    │
│  └───────────────────────┬─────────────────────────────────┘    │
│                          │                                      │
...
│  ┌─────────────────────────────────────────────────────┐  │
│  │  纠错阶段（Correction LLM）                         │  │
│  │  • 模型：本地 Llama-3.1-8B + LoRA（微调后）    │  │
│  │  • 输入：SQL + 执行结果/错误 + 用户问题         │  │
│  │  • 输出：needs_correction? + 纠正后 SQL          │  │
│  │                                                      │  │
│  │  流程：                                             │  │
│  │  1. 审查 LLM 判断是否需要纠正                     │  │
│  │  2. 如需纠正 → 应用修正 → 重新执行               │  │
...
│                        数据层 & 模型层                                 │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐   │
│  │  SQLite 数据库  │  │  生成模型      │  │  纠错模型      │   │
│  │  (dat/ 目录)  │  │  (API/Llama)  │  │  (Llama+LoRA) │   │
│  └────────────────┘  └────────────────┘  └────────────────┘   │
│                                                                      │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  记忆系统（Memory Manager）                                 │  │
│  │  • 短期记忆：当前对话历史                                │  │
│  │  • 长期记忆：ChromaDB 向量库（可选）                   │  │
│  │  • Schema 缓存：避免重复查询                            │  │
│  └─────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### 1. 意图检测（`src/agent/intent_detector.py`）

自动区分用户输入的意图，避免 Agent 回答无关问题：

- **SQL_QUERY**：需要查询数据库 → 进入 ReAct Agent
- **CHAT**：聊天范围（介绍自己、介绍数据库、解释 SQL 知识、总结历史）→ 知识库回答
- **REJECT**：无关问题 → 礼貌拒绝

使用 LLM Function Calling 风格进行分类，Fallback 为规则匹配。

### 2. ReAct Agent（`src/agent/react_agent.py`）

实现双模型协作的 ReAct (Reasoning + Acting) 循环：

```
生成 LLM (API/智谱 AI)          纠错 LLM (本地 Llama-3.1-8B + LoRA)
         │                                    ▲
         │ 生成 SQL                         │ 审查请求
         ▼                                    │
   SQL Executor ────────────────────→ 审查结果
...
**关键参数**（`config/agent_config.yaml`）：
- `max_iterations`：最大推理轮数（默认 5）
- `correction_threshold`：纠错置信度阈值（默认 0.3）
- `use_correction_model`：是否启用本地纠错模型

### 3. 多轮对话管理（`src/dialogue/dialogue_manager.py`）

- 集成意图检测，自动路由到对应处理逻辑
- 追问检测（基于规则和 LLM）
- 指代消解（"上一个"、"它" → 具体 SQL 或表名）
- 上下文维护、SQL 历史记录

### 4. SQL 生成（`src/agent/sql_generator.py`）

- 支持 API 大模型（智谱 AI）或本地小模型（Llama-3.1-8B）
- Few-shot Prompting
- Schema 感知生成

### 5. Clause 级纠错（`src/correction/clause_corrector.py`）
...
### 6. 长期记忆（`src/memory/memory_manager.py`）
